...
def normalize_quaternion(q):
    assert q.shape[-1] == 4
    return torch.nn.functional.normalize(q, p=2.0, dim=-1)
...
def forward_kinematics(rotations, root_positions, root, edges):
    ...
    get_offset(root)
    offsets = torch.tensor(offsets, device=torch.device('cuda:0'))
    expanded_offsets = offsets.expand(rotations.shape[0], rotations.shape[1], offsets.shape[0], offsets.shape[1])
    ...
    for i in range(offsets.shape[0]):
        if parents[i] == -1:
            ...
            if has_children[i]:
                counter += 1
                rotations_world.append(qmul(rotations_world[parents[i]], rotations[:, :, counter]))
            else:
